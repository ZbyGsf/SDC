...
class YOLO(object):
    _defaults = {
        ...
        "classes_path"      : 'model_data/marine_rubbish_classes.txt',
        #---------------------------------------------------------------------#
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        with torch.no_grad():
            ...
            if results[0] is None:
                ...
                forjingwei =np.array([])
                return image, classes_nums,forjingwei
                # return image

            top_label   = np.array(results[0][:, 6], dtype = 'int32')
            top_conf    = results[0][:, 4] * results[0][:, 5]
            top_boxes   = results[0][:, :4]
            #同时从results中取出top_label和top_boxes，放在一个数组中，方便后面的操作
            forjingwei = np.hstack((top_label.reshape(-1,1),top_boxes))
            print("forjingwei:",forjingwei)
        ...
        for i, c in list(enumerate(top_label)):
            ...
            del draw
        #前面还有一个return image，是为了在没有检测到目标的时候直接返回原图，所以两个return要同时修改才不会报错，否则在没有目标的时候，return只有一个返回值，而不是两个
        return image,classes_nums,forjingwei
        # return image
...
if __name__ == "__main__":
    ...
    device  = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    m       = YoloBody([[6, 7, 8], [3, 4, 5], [0, 1, 2]], 4).to(device)
    summary(m, input_size=(3, 1024, 576))